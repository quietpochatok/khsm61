...
RSpec.feature 'anon see profile users', type: :feature do
  ...
  let(:user) { create(:user, name: 'Вадик', email: 'vadik10@mail.ru') }
  # создаем игры, чтобы они "отрисовались" в профиле юзера
  let!(:games) do
    [
      create(:game, user: user, id: 15, created_at: Time.zone.parse('2016.10.09, 13:00'),
                         finished_at: Time.zone.parse('2016.10.09, 13:20'), current_level: 7, prize: 1000),
      create(:game, user: user, id: 26, created_at: Time.zone.parse('2020.12.01, 04:00'),
